...
class TPVImageCrossAttention(BaseModule):
    ...
    def forward(self,
        ...
        if residual is None:
            inp_residual = query
        bs, num_query, _ = query.size() # bs hw c
        ...
        for tpv_idx, tpv_mask in enumerate(tpv_masks): # 3 C_N B num_query D
            indexes = []
            for _, mask_per_img in enumerate(tpv_mask): # C_N B num_query D
                #  B num_query D -> num_query D -> num_query
            ...
            for i, reference_points_per_img in enumerate(reference_points_cam):
                for j in range(bs):
                    index_query_per_img = indexes[i]                                    # bs hw c
